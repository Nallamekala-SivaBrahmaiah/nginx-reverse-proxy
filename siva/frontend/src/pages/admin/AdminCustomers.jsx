...
const AdminCustomers = () => {
  ...
  const { data: usersRes, isLoading } = useQuery({
    ...
    queryFn: () => API.get('/users/admin/customers'),
  });

  const customers = usersRes?.data?.users || [];
            ...
            {customers.length === 0 ? (
              <tr>
                <td colSpan={6} className="p-6 text-center text-gray-500 font-semibold">
                  No customers registered on the platform.
                </td>
              </tr>
            ) : (
              customers.map((cust) => (
                <tr key={cust._id} className="hover:bg-gray-50/50 dark:hover:bg-zinc-850">
                  <td className="p-4 font-bold text-zinc-900 dark:text-white">{cust.name}</td>
                  <td className="p-4 font-semibold text-zinc-600 dark:text-zinc-400">{cust.email}</td>
                  <td className="p-4 font-bold text-zinc-700 dark:text-zinc-350">{cust.phone || 'N/A'}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-sm uppercase ${
                      cust.status === 'active' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'
                    }`}>
                      {cust.status}
                    </span>
                  </td>
                  <td className="p-4 text-center">
                    {cust.status === 'active' ? (
                      <button
                        onClick={() => updateStatusMutation.mutate({ id: cust._id, status: 'blocked' })}
                      ...
                      <button
                        onClick={() => updateStatusMutation.mutate({ id: cust._id, status: 'active' })}
